...
export function Unit({
  ...
  function handleSelect(lessonSlug: string): void {
    ...
    if (lesson.kc_id != null && lesson.kc_id !== '') {
      // The catalog's kc_id is the KC to launch. The frontend KnowledgeComponentId union is
      // narrower than the full catalog (only the live fraction KCs today), so we assert here; a
      // KC with no live lesson surfaces as the host's "couldn't start that lesson" toast (DEC.3).
